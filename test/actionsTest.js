...
describe('actions.js', () => {
	...
	describe('doTrim()', () => {
		it('exists', () => {
			assert.property(actions, 'doTrim');
			assert.isFunction(actions.doTrim);
		});

		it('ignores nulls', () => {
			let field = createFieldInstance(null);

			actions.doTrim(field, {});

			assert.equal(field.value, null);
		});

		it('ignores numbers', () => {
			let field = createFieldInstance(15);
		...
		it('trims spaces', () => {
			let field = createFieldInstance(' trim ');

			actions.doTrim(field, {});

			assert.equal(field.value, 'trim');
		});
		...
		it('trims carriage returns', () => {
			let field = createFieldInstance('\rtrim\r');
